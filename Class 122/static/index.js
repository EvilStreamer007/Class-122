    ...
    $("#send_button").click(function () {
        ...
        if (user_bot_input_text != "") {
            ...
            $.ajax({
                ...
                contentType: "application/json",
                success: function(result){
                    $("#chat_messages").append('<div class = "bot_messages">' + result.bot_response + "</div>") //combines messages from bot and user
                    $('.chatbot_messages_container').animate({
                        scrollTop: $(".chatbot_messages_container")[0].scrollHeight}, 1000) 
                },  
                error: function(result){
                    alert(result.responseJSON.message)
                }
